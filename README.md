...
- **👥 챌린지 인증 & 관리자 대시보드** — Firebase를 설정하면(아래 [Firebase 설정
  ...
  스스로 등록**되고 매일 읽기/듣기 인증을 제출합니다. 운영자는 관리자 화면에서 전체 참가자의
  인증 현황·누적 미인증 횟수·킥아웃 대상 여부를 확인하고, 면제일 등록과 킥아웃 처리를 할 수
  있습니다. 계정으로 본인이 확인되므로 남의 이름으로 인증할 수 없습니다. Firebase를
  설정하지 않으면 이 기능은 자동으로 숨겨집니다.
