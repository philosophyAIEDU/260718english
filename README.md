...
- **📖 Read the Library** — 계정·비용 없이 바로 읽을 수 있는 무료 고전 영어 원서 12종과
  영어 성경(World English Bible) 6권, 총 18종이 들어있습니다 (아래 목록 참고).
  Beginner(초록) · Intermediate(골드) · Advanced(자주) 레벨별로 색상 구분되어 묶여
  있고, 각 책마다 **챌린지 기간(28일) 기준 하루 페이지 수**가 표시됩니다. 리더 화면에서도
  지금이 28일 중 며칠째인지 확인할 수 있어요. 페이지를 넘기며 읽다가 "Analyze this page"를
  누르면 사진 없이 바로 학습 가이드가 만들어집니다.
...
- **✨ 현대식 영어 버전** — 옛 어휘·문장 구조가 많은 고전 원문을 AI로 현대식 영어로 다시 써서
  원문과 나란히 저장해 둘 수 있습니다 (아래 [현대식 영어 버전
  만들기](#-현대식-영어-버전-만들기) 참고). 준비된 책은 리더 화면에서 "📜 원문 그대로 / ✨
  현대식 영어" 토글로 바로 전환됩니다.
...
- **컬러풀한 책 표지** — 라이브러리의 18권이 각자 다른 색·이모지 표지를 가져서, 실제
...
19세기~20세기 초 고전 원문은 (thee/thou, shall/whilst 같은) 옛 어휘와 문장 구조가 많아서
영어 공부용으로는 부담스러울 수 있습니다. `scripts/modernize-books.mjs`가 각 책의 원문을
**AI로 현대식 영어로 다시 써서** 원문 옆에 함께 저장해 두면, 리더 화면에 "📜 원문 그대로 /
✨ 현대식 영어" 토글이 나타나 참가자가 골라 읽을 수 있습니다. (영어 성경 6권은 이미 현대
영어(WEB)라서 이 스크립트가 자동으로 건너뜁니다 — 대상은 고전 원서 12권입니다.)

```bash
# 본인의 Gemini API 키로 고전 원서 12권을 변환 (시간이 꽤 걸립니다 — 책 한 권이 소설 한 권 분량입니다)
GEMINI_API_KEY=여기에_키 npm run modernize

# 한 권만 먼저 해보고 싶다면
GEMINI_API_KEY=여기에_키 node scripts/modernize-books.mjs --book wizard-of-oz

# API 호출 없이 몇 페이지가 처리될지만 미리 보기
node scripts/modernize-books.mjs --dry-run
```

- **중간에 멈춰도 괜찮습니다.** `scripts/.modernize-progress.json`(커밋되지 않음)에 챕터별
  진행 상황이 저장되어, 같은 명령을 다시 실행하면 이어서 진행합니다.
- 결과는 각 책의 `chapters[].modernParagraphs`에 원문(`paragraphs`)과 나란히 저장됩니다 —
  **원문은 전혀 건드리지 않습니다.**
- 이 스크립트를 실행한 뒤 `public/books/*.json`의 변경사항을 커밋·배포하면, 참가자들은
  **실시간 API 호출 없이** 바로 현대식 영어 버전을 읽을 수 있습니다 (모두가 같은 미리 만들어진
  텍스트를 읽으므로 빠르고, API 비용도 한 번만 듭니다).
- 아직 변환하지 않은 챕터를 "현대식 영어"로 보면, 그 챕터만 자동으로 원문이 표시됩니다.
