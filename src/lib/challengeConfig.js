...
export const CHALLENGE_CONFIG = {
  ...
  // 챌린지 기간 (양 끝 포함). 7일 × 4주 = 28일이라, 아래 CURRICULUM의
  // 4회 수업과 주차가 정확히 맞아떨어집니다. (인증 자체는 평일만 요구되지만
  // — excludeWeekends 참고 — 리딩 진도·주차 계산은 이 28일 달력 그대로입니다.)
  ...
  // true면 토·일요일도 매주 자동으로 면제됩니다 — holidays와 같은 방식으로
  // 처리되어(challengeUtils.isHoliday), 앞으로의 주말은 물론 이미 지난
  // 주말에 미인증(X)으로 잡혔던 기록도 다시 계산되는 즉시 면제(P)로
  // 바뀝니다(과거 기록을 따로 고치거나 참가자별로 면제 처리할 필요 없음).
  // 평일 기준 주 5일 인증이 됩니다. 챌린지 전체 기간(startDate~endDate)과
  // 리딩 진도(Day 1~28, 4주 커리큘럼)는 그대로 유지되고, 인증 요구만
  // 평일로 제한됩니다.
...
export const PILLARS = [
  {
    icon: 'book',
    title: '주 5일 듣기 인증',
    body: '평일(월~금)마다, 영어 원문을 눈으로 읽으면서 들어도 되고 듣기만 해도 됩니다 — 다만 인증은 그날 mp3를 실제로 재생해야만 됩니다. 주말은 인증이 필요 없습니다.',
  },
  {
    icon: 'sparkles',
    title: 'AI 학습 가이드',
...
export const RULES = [
  ...
  },
  {
    icon: 'check',
    title: '주 5일 인증 (평일만)',
    body: '월~금 매일 홈 화면에서 그날 분량의 mp3를 실제로 들으면 인증됩니다. 마감은 매일 자정(24시)입니다. 토·일요일은 인증이 필요 없습니다.',
  },
  {
    icon: 'speaker',
