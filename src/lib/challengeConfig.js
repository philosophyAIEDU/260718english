...
export const CHALLENGE_CONFIG = {
  ...
  // 운영자 대시보드의 "공지 문구"에 링크로 넣을 배포 주소. 비워두면 링크 줄이
  // 그냥 빠집니다 — Netlify 등에 배포한 뒤 실제 주소로 채워주세요.
  appUrl: '',
  ...
  // 전원 공통으로 인증이 면제되는 날짜(공휴일 등). 이 날짜들은 인증하지 않아도
  // 미인증(X)으로 집계되지 않고 스트릭도 끊기지 않습니다 — 참가자별 개별 면제
  // (관리자 화면의 "오늘 면제")와 같은 방식으로 처리되되, 명단 전체에 자동으로
  // 적용됩니다. 원한다면 그래도 인증할 수 있고, 그러면 평소처럼 인증(O)으로
  // 기록됩니다. YYYY-MM-DD 형식.
  holidays: ['2026-09-24', '2026-09-25', '2026-09-26', '2026-09-27'], // 추석 연휴
...
export const PILLARS = [
  {
    icon: 'book',
    title: '매일 듣기 인증',
    body: '주 7일, 영어 원문을 눈으로 읽으면서 들어도 되고 듣기만 해도 됩니다 — 다만 인증은 그날 mp3를 실제로 재생해야만 됩니다.',
  },
  {
    icon: 'sparkles',
    title: 'AI 학습 가이드',
...
export const RULES = [
  {
    icon: 'users',
    title: '구글 로그인 후 닉네임 설정',
    body: '홈 화면에서 구글 계정으로 로그인하고 닉네임만 정하면 바로 참여됩니다. 별도 신청 절차는 없습니다.',
  },
  {
    icon: 'check',
    title: '주 7일 인증',
    body: '매일 홈 화면에서 그날 분량의 mp3를 실제로 들으면 인증됩니다. 마감은 매일 자정(24시)입니다.',
  },
  {
    icon: 'speaker',
    title: '인증은 오늘 mp3 듣기로만',
    body: '라이브러리에서 그날 분량의 듣기 파일을 실제로 재생해야 인증됩니다. 읽기만 해서는 인증되지 않고, 버튼만 눌러서도 인증되지 않습니다.',
  },
  {
    icon: 'alert',
    title: '미인증 6회 = 킥아웃 대상',
  ...
  {
    icon: 'calendar',
    title: '추석 연휴(9/24~9/27)는 인증 제외',
    body: '이 기간에는 인증하지 않아도 미인증으로 집계되지 않고 스트릭도 끊기지 않습니다. 그래도 들으셨다면 평소처럼 인증하셔도 됩니다.',
  },
];
